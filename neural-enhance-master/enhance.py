...
class DataLoader(threading.Thread):
    ...
    def __init__(self):
        ...
        # print("original shape = ", self.orig_shape)
    ...
    def add_to_buffer(self, f):
        ...
        try:
            ...
            # print("original shape2 = ", orig.shape)
            ...
            if scale > 1 and all(s // scale >= args.batch_shape for s in orig.size):
                ...
                # print("original shape after scaling = ", orig.shape)
        ...
        if args.train_blur is not None:
            ...
            # print('seed after blur = ',seed)
            # print('seed shape after bluring = ', seed.shape)
        if args.zoom > 1:
            seed = seed.resize((orig.size[0] // args.zoom, orig.size[1] // args.zoom), resample=PIL.Image.LANCZOS)
            # print('seed after zoom = ',seed)
            # print('seed shape after zooming = ', seed.shape)
        if len(args.train_jpeg) > 0:
            ...
            # print('seed after train_jpeg > 0 = ',seed)
            # print('seed shape after train_jpeg > 0 =', seed.shape)
        ...
        if args.train_noise is not None:
            ...
            # print('seed after noise = ',seed)
            # print('seed shape after noise = ', seed.shape)
        for _ in range(seed.shape[0] * seed.shape[1] // (args.buffer_fraction * self.seed_shape ** 2)):
            ...
            while len(self.available) == 0:
                ...
                # print('available list size before clear:',len(self.available))
                # print('available list before clear: ',self.available)
                self.data_copied.clear()
                # print('available list size after clear:',len(self.available))
                # print('available list after clear: ',self.available)
    ...
    def copy(self, origs_out, seeds_out):
        ...
        for i, j in enumerate(random.sample(self.ready, args.batch_size)):
            ...
            # print('origs_out[i] = ',origs_out[i])
            seeds_out[i] = self.seed_buffer[j]
            # print('seeds_out[i] = ',seeds_out[i])
...
class SubpixelReshuffleLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        print("in get_output_for function")
        out, r = T.zeros(self.get_output_shape_for(input.shape)), self.upscale
        # print("input shape for the prev output shape: ",input.shape)
        # print('out :',out)
        print('itertools.product(range(r), repeat=2): ', itertools.product(range(r), repeat=2))
        for y, x in itertools.product(range(r), repeat=2):
            out = T.inc_subtensor(out[:, :, y::r, x::r], input[:, r * y + x::r * r, :, :])
        # print('out in loop: ',out)
        return out


class Model(object):
    # print("here is the Model class :D")
    # print('object: ',object)
    def __init__(self):
        self.network = collections.OrderedDict()
        self.network['img'] = InputLayer((None, 3, None, None))
        #   print("self.network['img']: ",self.network['img'])
        self.network['seed'] = InputLayer((None, 3, None, None))
        #  print("self.network['seed']: ",self.network['seed'])
    ...
    def last_layer(self):
        # print('last_layer func output: ',list(self.network.values())[-1])
    ...
    def setup_discriminator(self):
        ...
        # print("self.network['disc']: ",self.network['disc'])
...
class NeuralEnhancer(object):
    ...
    def train(self):
        ...
        # print("lr initial value: ",learning_rate)
        try:
            average, start = None, time.time()
            #   print("epochs training loop :D")
            for epoch in range(args.epochs):
                ...
                for _ in range(args.epoch_size):
                    ...
                    #     print("there is the epoch size loop :", args.epoch_size )
                    ...
                    l = np.sum(losses)
                    #  print('l i think its also a loss: ',l)
    ...
    def process(self, original):
        ...
        # print('image size after padding: ',image.shape)
...
if __name__ == "__main__":
    if args.train:
        ...
        # print('if enhancer: ',enhancer)
        enhancer.train()
    else:
        enhancer = NeuralEnhancer(loader=False)
        # print('else enhancer: ',enhancer)
        for filename in args.files:
            print(filename, end=' ')
            img = scipy.ndimage.imread(filename, mode='RGB')
            #   print('first img : ',img)
            #  print('first img shape : ',img.shape)
